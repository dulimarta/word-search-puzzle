...
//        /* Using a traditional for-loop the above code is */
...
void WordPuzzleSolver::solve_with_quad() const {
     ...
     * the first two integers in the tuple are the row,col positions
     * the third is ACROSS/DOWN */
    vector<tuple<int,int,string>> solution (all_words.size());
    for (int k = 0; k < grid.size(); k++) {
        for (int m = 0; m < grid[0].size(); m++) {
            /* we have to check BOTH directions */
